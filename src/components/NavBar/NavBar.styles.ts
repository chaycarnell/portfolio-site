...
const transition = css<ResponsiveLayoutProps>`
  ${({ $showHeader }) =>
    $showHeader ? 'max-height 0.15s ease-in' : 'max-height 0.15s ease-out'};
`;

const maxHeight = css<ResponsiveLayoutProps>`
  ${({ $showHeader }) => ($showHeader ? '210px' : '48px')};
`;

export const Wrapper = styled.div<ResponsiveLayoutProps>`
...
export const TopContent = styled.div<ResponsiveLayoutProps>`
  display: grid;
  background: var(--primary);
...
export const BottomContent = styled.div<ResponsiveLayoutProps>`
